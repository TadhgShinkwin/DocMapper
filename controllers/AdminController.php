...
/**
 * Class: AdminController now admin controller
 * This is the controller for the ADMIN user type
 *
 * 
 * 
 */
    ...
    public function updateView() { //update the VIEW based on the users page selection
        if (isset($this->getArray['pageID'])) { //check if a page id is contained in the URL
            switch ($this->getArray['pageID']) {
                ...
                case "usermgmt"://to be constructed. 

                    $usermgmt = new Usermgmt($this->user, $this->postArray, $this->pageTitle, strtoupper($this->getArray['pageID']), $this->db);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$usermgmt,$navigation);
                    $data['menuNav'] = $navigation->getMenuNav();
                    $data['pageTitle'] = $usermgmt->getPageTitle();
                    $data['pageHeading'] = $usermgmt->getPageHeading();
                    $data['panelHeadLHS'] = $usermgmt->getPanelHead_1(); // A string containing the LHS panel heading/title
                    $data['stringLHS'] = $usermgmt->getPanelContent_1();     // A string intended of the Left Hand Side of the page
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purposes
                    //update the view
